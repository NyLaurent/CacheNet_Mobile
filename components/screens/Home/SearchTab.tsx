...
const CacheManager = ({ navigation }: { any}) => {
  ...
  const fetchCachedData = async () => {
    ...
    try {
      ...
      const response = await axios.get(`http://10.0.2.2:3000/cache`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      setItems(response.data);
    } catch (error) {
      console.error("Error fetching cache data:", error);
      Alert.alert("Error", "Failed to load cache data. Please try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <View style={{ flex: 1, padding: 20, backgroundColor: "#f8f8f8" }}>
      <Text style={{ fontSize: 22, fontWeight: "bold", marginBottom: 10 }}>Cache Manager</Text>
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <TouchableOpacity
              style={{ padding: 15, backgroundColor: "#fff", marginBottom: 10, borderRadius: 5 }}
              onPress={() => navigation.navigate("CacheDetails", { item })}
            >
